...
build:: tfgen provider
	cd provider && for LANGUAGE in "nodejs" "python" "go" ; do \
...
tfgen::
	cd provider && go install -ldflags "-X github.com/timmyers/pulumi-${PACK}/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${TFGEN}
...
provider:: generate_schema
	...
	cd provider && go install -ldflags "-X github.com/timmyers/pulumi-${PACK}/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${PROVIDER}
